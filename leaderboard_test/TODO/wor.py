...
import sys
sys.path.append("D:\Code\.SOTA\WorldOnRails\\autoagents")
...
def main():
    ...
    agent = ImageAgent(role_name=actor_config.rolename, carla_host="localhost", carla_port=2000)
    ...
    env = mats_gym.route_scenario_env(
        ...
        sensor_specs={"hero": agent.sensors()},  # sensor specs for each agent added with **kwargs
    )
    ...
    for _ in range(100):
        ...
        agent.setup(path_to_conf_file="D:\Code\.SOTA\WorldOnRails\leaderboard_weights\config_leaderboard.yaml", route=env.current_actor_config.route)
