...
import re # search for index

SECONDS_GIVEN_PER_METERS = 0.4

# Changed from RouteScenario, Let different vehicles to drive with certain routes
class RouteParallel(BasicScenario):

    """
    Implementation of a RouteParallel, i.e. a scenario that consists of driving along some of the pre-defined routes
    """

    def __init__(self, world, config, debug_mode=False, criteria_enable=False, timeout=300):
        """
        ...
        # TODO randomrize the route for single agent
        # selected_numbers = random.sample(range(len(config.keypoints)), len(config.ego_vehicles))
        for actor_config in config.ego_vehicles:
            _rolename = actor_config.rolename
            self.route[_rolename] = self._get_route(config) 
            ego_vehicle = self._spawn_ego_vehicle(_rolename)

            if ego_vehicle is None: continue

            sampled_scenario_definitions = self._filter_scenarios(config.scenario_configs, _rolename)
            
            if debug_mode:
                self._draw_waypoints(world, self.route[_rolename], vertical_shift=0.1, size=0.1, persistency=timeout, downsample=5)

            self._build_scenarios(
                world, ego_vehicle, sampled_scenario_definitions, timeout=timeout, debug=debug_mode > 0
            )
    ...
    def _get_route(self, config):
        ...
        if config.agent is not None:
            for agent_id, agent_ins in config.agent.items():
                match = re.search(r"hero_(\d+)", agent_id)
                if match:
                    agent_id = str(match.group(1))
                gps_route, route = interpolate_trajectory(config.keypoints[agent_id])
                agent_ins.set_global_plan(gps_route, route)

        return route

    def _filter_scenarios(self, scenario_configs, rolename):
        """
        ...
        for scenario_config in scenario_configs:
            ...
            if not RouteParser.is_scenario_at_route(trigger_point, self.route[rolename]):
    ...
    def _spawn_ego_vehicle(self, rolename):
        """Spawn the ego vehicle at the first waypoint of the route"""
        elevate_transform = self.route[rolename][0][0]
    ...
    def _build_scenarios(self, world, ego_vehicle, scenario_definitions, scenarios_per_tick=5, timeout=300, debug=False, rolename="hero"):
        """
        ...
        self.list_scenarios = []
        ego_data = ActorConfigurationData(ego_vehicle.type_id, ego_vehicle.get_transform(), rolename)
        ...
        for scenario_number, scenario_config in enumerate(scenario_definitions):
            scenario_config.ego_vehicles.append(ego_data)
            scenario_config.route_var_name.append("ScenarioRouteNumber{}".format(scenario_number))
            scenario_config.route.append(self.route[rolename])
            try:
